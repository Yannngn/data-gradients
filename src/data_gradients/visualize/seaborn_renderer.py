...
RenderDataType: TypeAlias = pd.DataFrame | np.ndarray | Figure


class SeabornRenderer(PlotRenderer):
    def __init__(self, style: str = "whitegrid", palette=PALETTE_NAME):
        seaborn.set_theme(style=style, palette=palette)  # type: ignore[argType]

    def render(self, data: RenderDataType | None, options: CommonPlotOptions) -> Figure | None:
        ...
        if data is None:
            return None
        if isinstance(options, Hist2DPlotOptions) and isinstance(data, pd.DataFrame):
            return self._render_histplot(data, options)
        if isinstance(options, BarPlotOptions) and isinstance(data, pd.DataFrame):
            return self._render_barplot(data, options)
        if isinstance(options, ScatterPlotOptions) and isinstance(data, pd.DataFrame):
            return self._render_scatterplot(data, options)
        if isinstance(options, ViolinPlotOptions) and isinstance(data, pd.DataFrame):
            return self._render_violinplot(data, options)
        if isinstance(options, KDEPlotOptions) and isinstance(data, pd.DataFrame):
            return self._render_kdeplot(data, options)
        if isinstance(options, FigureRenderer) and isinstance(data, Figure):
            return self._render_figure(data, options)
        if isinstance(options, HeatmapOptions) and isinstance(data, Mapping):
            return self._render_heatmap(data, options)

        raise ValueError(f"Unknown options type: {type(options)}")

    def _render_scatterplot(self, df: pd.DataFrame, options: ScatterPlotOptions) -> Figure:
        ...
        subplots: tuple[Figure, np.ndarray] = plt.subplots(
            n_rows,
            n_cols,
            sharey=options.sharey,  # type: ignore[argType]
        ...
        for df, ax_i in zip(dfs, axs, strict=False):
            assert isinstance(ax_i, Axes)  # only for typing hint
            scatterplot_args = {
                "data": df,
    ...
    def _render_histplot(self, df: pd.DataFrame, options: Hist2DPlotOptions) -> Figure:
        if options.individual_plots_key is None:
            dfs = [df]
            n_rows = 1
            n_cols = 1
        ...
        for df, ax_i in zip(dfs, axs, strict=False):
            assert isinstance(ax_i, Axes)  # only for typing hint
            histplot_args = {"data": df, "x": options.x_label_key, "kde": options.kde, "stat": options.stat, "ax": ax_i}
    ...
    def _render_kdeplot(self, df: pd.DataFrame, options: KDEPlotOptions) -> Figure:
        if options.individual_plots_key is None:
            dfs = [df]
            n_rows = 1
            n_cols = 1
    ...
    def _render_violinplot(self, df: pd.DataFrame, options: ViolinPlotOptions) -> Figure:
        ...
        if options.bandwidth is not None:
            plot_args.update(bw_method=options.bandwidth)
    ...
    def _render_barplot(self, df: pd.DataFrame, options: BarPlotOptions) -> Figure:
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=options.figsize)
        fig.subplots_adjust(top=0.9)
    ...
    def _render_heatmap(self, data: Mapping[str, np.ndarray], options: HeatmapOptions) -> Figure:
    ...
    def _render_figure(self, fig: Figure, options: FigureRenderer) -> Figure:
    ...
    def _set_ticks_rotation(self, ax: Axes, x_ticks_rotation: float | None, y_ticks_rotation: float | None):
    ...
    def _show_values(self, axs: np.ndarray | Axes, orient="v", space=0.01):
        def _single(ax: Axes):
            if orient == "v":
                for p in ax.patches:
                    assert isinstance(p, mpatches.Rectangle)
