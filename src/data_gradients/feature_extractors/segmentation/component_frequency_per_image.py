...
class SegmentationComponentsPerImageCount(NoSourceFeatureExtractor):
    """
    Calculates and visualizes the number of distinct segmented components per image across different dataset splits.

    This feature extractor counts the total number of segmented components (objects) in each image,
    which can provide insights into the complexity of
    the scenes within the dataset. It can help identify if there is a balance or imbalance in the
    number of objects per image across the training and
    validation sets.
    Understanding this distribution is important for adjusting model hyperparameters that may depend on
    the expected number of objects in a scene,
    ...
    def aggregate(self) -> Feature:
        ...
        json = {
            "train": df[df["split"] == "train"]["n_components"].describe().to_dict(),
            "val": df[df["split"] == "val"]["n_components"].describe().to_dict(),
        }
