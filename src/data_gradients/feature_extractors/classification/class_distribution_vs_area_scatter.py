...
class ClassificationClassDistributionVsAreaPlot(NoSourceFeatureExtractor):
    """
    Visualizes the spread of image widths and heights within each class and across data splits.

    This feature extractor creates a scatter plot to graphically represent the diversity of image dimensions associated with each class label
    and split in the dataset.
    By visualizing this data, users can quickly assess whether certain classes or splits contain images that are consistently larger or smaller
    than others,
    ...
    def aggregate(self) -> Feature:
        ...
        feature = Feature(
            ...
            description=(
                "Distribution of image size (mean value of image width & height) with respect to assigned image label and (when possible) "
                "a split.\n"
                "This may highlight issues when classes in train/val has different image resolution which may negatively affect the accuracy "
                "of the model.\n"
                "If you see a large difference in image size between classes and splits - you may need to adjust data collection process or "
                "training regime:\n"
